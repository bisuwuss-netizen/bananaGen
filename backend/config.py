...
class Config:
    ...
    # 图片生成提供者: qwen / dashscope / gemini / google / mock / test
    IMAGE_PROVIDER = os.getenv('IMAGE_PROVIDER', 'qwen')
    IMAGE_PROVIDER_DEFAULT = os.getenv('IMAGE_PROVIDER_DEFAULT', 'qwen')
    
    # 阿里云 DashScope API 配置
    DASHSCOPE_API_KEY = os.getenv('DASHSCOPE_API_KEY', '')
    DASHSCOPE_IMAGE_MODEL = os.getenv('DASHSCOPE_IMAGE_MODEL', 'qwen-image-plus')
    DASHSCOPE_API_BASE = os.getenv('DASHSCOPE_API_BASE', 'https://dashscope.aliyuncs.com/api/v1/services/aigc/text2image/image-synthesis')
    DASHSCOPE_TASK_API_BASE = os.getenv('DASHSCOPE_TASK_API_BASE', 'https://dashscope.aliyuncs.com/api/v1/tasks')
    DASHSCOPE_DEFAULT_SIZE = os.getenv('DASHSCOPE_DEFAULT_SIZE', '1792*1024')
    DASHSCOPE_TIMEOUT = int(os.getenv('DASHSCOPE_TIMEOUT', '30'))
    DASHSCOPE_MAX_POLL_WAIT = int(os.getenv('DASHSCOPE_MAX_POLL_WAIT', '120'))
    DASHSCOPE_POLL_INTERVAL = int(os.getenv('DASHSCOPE_POLL_INTERVAL', '2'))
    DASHSCOPE_NEGATIVE_PROMPT = os.getenv('DASHSCOPE_NEGATIVE_PROMPT', '')
    
    # HTML 渲染离线模式（启用后会内联 CSS）
    OFFLINE_MODE = os.getenv('OFFLINE_MODE', 'false').lower() == 'true'
    
    # 默认教法模式
    DEFAULT_PEDAGOGY_METHOD = os.getenv('DEFAULT_PEDAGOGY_METHOD', 'five_step')
    
    # 默认职教模板
    DEFAULT_VOCATIONAL_TEMPLATE = os.getenv('DEFAULT_VOCATIONAL_TEMPLATE', 'theory_professional')
