...
def list_projects():
    ...
    try:
        ...
        # 历史项目列表需要包含完整的页面数据（包括 outline_content 和 description_content）
        # 这样用户打开历史项目时就能看到之前生成的大纲和内容
        return success_response({
            'projects': [project.to_dict(include_pages=True, page_summary=False) for project in projects],
            'has_more': has_more,
            'limit': limit,
            'offset': offset
        })
...
def create_project():
    ...
    try:
        ...
        if render_mode not in ['image', 'html']:
            render_mode = 'image'
        logger.info(f"create_project - render_mode: {render_mode}")

        # 获取 scheme_id，根据 render_mode 设置默认值
        scheme_id = data.get('scheme_id')
        if not scheme_id:
            # HTML 模式默认使用 edu_dark，image 模式默认使用 tech_blue
            scheme_id = 'edu_dark' if render_mode == 'html' else 'tech_blue'
        
        # 验证 scheme_id 是否在 LAYOUT_SCHEMES 中
        if scheme_id not in LAYOUT_SCHEMES:
            logger.warning(f"create_project - Invalid scheme_id: {scheme_id}, falling back to default")
            scheme_id = 'edu_dark' if render_mode == 'html' else 'tech_blue'
        
        logger.info(f"create_project - scheme_id: {scheme_id}, creation_type: {creation_type}")
        ...
        logger.error(f"create_project error trace: {error_trace}")
        # 返回更详细的错误信息，但不在生产环境暴露敏感信息
        error_message = str(e) if current_app.debug else "创建项目时发生错误，请查看服务器日志"
        return error_response('SERVER_ERROR', error_message, 500)
...
def get_project(project_id):
    ...
    try:
        ...
        # 获取项目详情时，包含完整的页面数据（包括 outline_content 和 description_content）
        return success_response(project.to_dict(include_pages=True, page_summary=False))
...
def update_project(project_id):
    ...
    try:
        ...
        logger.error(f"update_project error trace: {error_trace}")
        # 返回更详细的错误信息，但不在生产环境暴露敏感信息
        error_message = str(e) if current_app.debug else "更新项目时发生错误，请查看服务器日志"
        return error_response('SERVER_ERROR', error_message, 500)
...
def generate_html_images(project_id: str):
    ...
    from services.ai_providers.image.dashscope_provider import DashScopeImageProvider
    ...
    def generate():
        """SSE 生成器"""
        # 初始化 DashScope 图片生成器，使用配置中的模型
        from config import get_config
        config = get_config()
        image_provider = DashScopeImageProvider(model=config.IMAGE_MODEL)
