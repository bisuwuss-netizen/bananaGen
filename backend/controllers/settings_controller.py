...
def _sync_settings_to_config(settings: Settings):
    ...
    # Sync worker settings (ensure integer type - DB column may be VARCHAR)
    current_app.config["MAX_DESCRIPTION_WORKERS"] = int(settings.max_description_workers) if settings.max_description_workers else 5
    current_app.config["MAX_IMAGE_WORKERS"] = int(settings.max_image_workers) if settings.max_image_workers else 8
