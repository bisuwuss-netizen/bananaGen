...
def _get_materials_list(filter_project_id: str, user_id: str = None):
    """
    Common logic to get materials list.
    Returns (materials_list, error_response)
    Filters out orphaned records whose files no longer exist on disk.
    """
    ...
    for material in materials:
        abs_path = Path(file_service.get_absolute_path(material.relative_path))
        if abs_path.exists():
            valid_materials.append(material)
        else:
            orphan_ids.append(material.id)
            logger.warning(f"Orphaned material record {material.id}: file not found at {abs_path}")
    
    # 自动清理孤儿记录
    if orphan_ids:
        logger.info(f"Cleaning up {len(orphan_ids)} orphaned material records")
        Material.query.filter(Material.id.in_(orphan_ids)).delete(synchronize_session=False)
        try:
            db.session.commit()
        except Exception:
            db.session.rollback()
    
    materials_list = [material.to_dict() for material in valid_materials]
...
def _save_material_file(file, target_project_id: Optional[str]):
    ...
    logger.info(f"_save_material_file - user_id: {user_id}")
...
def generate_material_image(project_id):
    ...
    try:
        ...
        # 从cookie获取user_id
        user_id = request.cookies.get('user_id') or '1'
        logger.info(f"generate_material_image - user_id: {user_id}")
...
def list_materials(project_id):
    ...
    Query params:
        - user_id: Filter by user_id (optional, from cookie if not provided)
...
def list_all_materials():
    ...
    Query params:
        ...
        - user_id: Filter by user_id (optional, from query param or cookie)
    ...
    try:
        ...
        logger.info(f"list_all_materials - found {len(materials_list)} materials for user_id: {user_id}, project_id: {filter_project_id}")
