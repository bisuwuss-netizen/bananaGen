...
__all__ = [
    ...
    'ImageProvider', 'GenAIImageProvider', 'OpenAIImageProvider', 'DashScopeImageProvider', 'QwenImageProvider',
...
def get_image_provider(model: str = "gemini-3-pro-image-preview") -> ImageProvider:
    ...
    # Qwen image models (e.g. qwen-image-2.0-pro) use DashScope native multimodal-generation API.
    # Route to dedicated provider to avoid incompatible OpenAI images/chat payload mismatches.
    if model_lower.startswith('qwen-image'):
        logger.info(f"Using Qwen native provider for qwen image model: {model}")
        api_key = (
            _get_config_value('DASHSCOPE_API_KEY')
            or _get_config_value('OPENAI_API_KEY')
            or _get_config_value('GOOGLE_API_KEY')
        )
        api_base = (
            _get_config_value('DASHSCOPE_API_BASE')
            or _get_config_value('OPENAI_API_BASE')
            or 'https://dashscope.aliyuncs.com/api/v1'
        )
        if not api_key:
            raise ValueError("DASHSCOPE_API_KEY / OPENAI_API_KEY / GOOGLE_API_KEY is required for qwen-image models")
        return QwenImageProvider(api_key=api_key, api_base=api_base, model=model)

    # DashScope (百炼) - wanx 系列模型
    if model and model_lower.startswith('wanx'):
        logger.info(f"Using DashScope (Bailian) for image generation, model: {model}")
        api_key = _get_config_value('DASHSCOPE_API_KEY') or _get_config_value('OPENAI_API_KEY') or _get_config_value('GOOGLE_API_KEY')
        api_base = _get_config_value('DASHSCOPE_API_BASE', 'https://dashscope.aliyuncs.com/api/v1')
        if not api_key:
            raise ValueError("DASHSCOPE_API_KEY is required for wanx-v1 image generation")
        return DashScopeImageProvider(api_key=api_key, api_base=api_base, model=model)
