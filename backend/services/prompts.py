...
VOCATIONAL_EDUCATION_CONTEXT = """
## 高职教育场景特征
- 教学场景：理论课(theory) / 实训课(practice) / 复习课(review)
- 专业类别：工程技术类 / 医药卫生类 / 农林牧渔类 / 信息技术类 / 财经商贸类 等
- 内容特点：理论与实践结合、步骤操作导向、案例驱动、技能培养
- 受众特点：高职学生，注重实用性和可操作性

## 页面类型建议
- 封面页(title)：突出课程名称和专业特色
- 目录页(toc)：清晰的章节导航
- 导入页(intro)：情境创设、问题引入
- 概念页(concept)：概念定义 + 配图说明
- 步骤页(steps)：左图右步骤的实训布局
- 对比页(comparison)：正误对比、方案对比
- 习题页(exercises)：巩固练习 + 答案解析
- 总结页(summary)：知识点回顾、思政升华
"""


def get_vocational_outline_generation_prompt(
    project_context: 'ProjectContext',
    language: str = "zh",
    teaching_scene: str = "theory",
    pedagogy_method: str = "five_step",
    include_ideology: bool = True,
    theory_practice_ratio: float = 0.5
) -> str:
    """
    生成职教场景大纲的 Prompt（模板化PPT生成专用）
    
    Args:
        project_context: 项目上下文
        language: 语言 (zh / en)
        teaching_scene: 教学场景 (theory / practice / review)
        pedagogy_method: 教法模式 ID
        include_ideology: 是否融入课程思政
        theory_practice_ratio: 理实比例 (0=纯理论, 1=纯实训)
    """
    ...
    # 语言指令
    lang_instruction = "用简体中文输出所有内容" if language == "zh" else "Output all content in English"
    ...
    if include_ideology:
        ideology_instruction = """
## 课程思政要求
1. 在教学目标中融入情感/素质目标（如工匠精神、职业道德、安全意识）
2. 在总结页适当升华主题，关联行业责任感、社会价值
3. 在案例选取时优先选用体现中国智慧、大国工匠的正面案例
"""

    # 理实比例指令
    ratio_instruction = ""
    if theory_practice_ratio < 0.3:
        ratio_instruction = "侧重理论讲解，多使用概念页、对比页"
    elif theory_practice_ratio > 0.7:
        ratio_instruction = "侧重实训操作，多使用步骤页、演示页"
    else:
        ratio_instruction = "理实结合，理论与实操穿插进行"
...
## 教法模式
{pedagogy.prompt_guidance}

## 教学场景
当前场景：{teaching_scene}
理实比例：{ratio_instruction}
...
{{
  "deck_title": "课程标题",
  "total_slides": 10,
  "pedagogy_method": "{pedagogy_method}",
  "slides": [
    {{
      "index": 1,
      "slide_type": "title",
      "title": "具体的页面标题",
      "bullets": ["要点1", "要点2"],
      "pedagogy_phase": "{pedagogy.structure[0] if pedagogy.structure else ''}"
    }}
  ]
}}
```
...
**注意**：
1. 每个 bullets 项必须是完整的、有实际内容的句子，禁止使用占位符
2. slide_type 必须从 [title, toc, intro, concept, steps, comparison, exercises, summary] 中选择
3. pedagogy_phase 必须填写当前页对应的教法环节名称
"""
