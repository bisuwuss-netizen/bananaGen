  ...
  return (
    <div className={isCompactMode ? `group ${className}` : `group bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-3 md:p-4 border border-purple-200 ${className}`}>
      ...
      {!isCompactMode && (
        <div className="flex items-center justify-between mb-2 md:mb-3">
          <div className="flex items-center gap-2">
            <Sparkles size={16} className="text-banana-500 md:w-[18px] md:h-[18px]" />
          ...
          {history.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 text-xs text-banana-500 hover:text-banana-500 transition-colors"
            >
              <History size={14} />
          ...
          <ul className="space-y-1">
            {history.map((req, idx) => (
              <li key={idx} className="text-xs text-gray-700 flex items-start gap-1">
                <span className="text-banana-500 flex-shrink-0">{idx + 1}.</span>
      ...
      <div className="flex gap-2 items-center relative">
        ...
        {isCompactMode && (
          <>
            <Sparkles size={16} className={`flex-shrink-0 transition-colors ${isSubmitting ? 'text-banana-500' : 'text-banana-500'}`} />
            {history.length > 0 && (
              <button
                onClick={() => setShowHistory(!showHistory)}
                className="flex items-center gap-1 text-xs text-gray-500 hover:text-banana-500 transition-colors flex-shrink-0"
        ...
        <div className="flex-1 relative">
          <input
            ...
            className={`w-full px-3 py-1.5 text-sm border ${isCompactMode ? 'border-gray-200' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-banana-500 focus:border-transparent transition-all ${
        ...
        <button
          ...
          className={`flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-lg transition-all ${
            !requirement.trim() || isSubmitting
              ...
              : 'bg-banana-500 text-white hover:bg-banana-500 active:scale-95'
