...
interface ProjectState {
  ...
  taskProgress: { total: number; completed: number; current_page?: string; stage?: string; [key: string]: any } | null;
  error: string | null;
  ...
  // 项目操作
  initializeProject: (type: 'idea' | 'outline' | 'description', content: string, templateImage?: File, templateStyle?: string, renderMode?: 'image' | 'html', schemeId?: string) => Promise<void>;
  syncProject: (projectId?: string) => Promise<void>;
  
  // 页面操作
  updatePageLocal: (pageId: string, data: any) => void;
  flushPendingUpdates: () => void;  // 立即提交所有待保存的更改
...
const debouncedUpdatePage = debounce(
  ...
  // 立即提交所有待保存的更改（flush防抖）
  flushPendingUpdates: () => {
    debouncedUpdatePage.flush();
  },

  // 立即保存所有页面的更改（用于保存按钮）
  // 先flush防抖，然后同步项目状态以确保updated_at更新
  saveAllPages: async () => {
    const { currentProject } = get();
    if (!currentProject) return;

    // 立即flush所有待保存的更改，并等待API调用完成
      ...
      const pollAndSync = async () => {
        try {
          ...
          if (task) {
            ...
            // Only full sync when new pages completed (avoid redundant DB queries)
            const newCompleted = task.progress?.completed ?? 0;
