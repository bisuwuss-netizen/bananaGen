 ...
 * 职教模板定义
 */
export interface VocationalTemplate {
  id: string;
  name: string;
  description: string;
 ...
 * 教法模型定义
 */
export interface PedagogyModel {
  id: string;
  name: string;
  name_en: string;
  description: string;
...
/**
 * 获取所有可用模板
 * @param scene 可选，按场景筛选 (theory / practice / review)
 */
export const listVocationalTemplates = async (scene?: string): Promise<ApiResponse<{ templates: VocationalTemplate[]; total: number }>> => {
  const url = scene ? `/api/templates?scene=${scene}` : '/api/templates';
  const response = await apiClient.get<ApiResponse<{ templates: VocationalTemplate[]; total: number }>>(url);
  return response.data;
};

/**
 * 获取所有可用教法模型
 * @param scene 可选，按场景筛选 (theory / practice / review / mixed)
 */
export const listPedagogies = async (scene?: string): Promise<ApiResponse<{ pedagogies: PedagogyModel[]; total: number }>> => {
  const url = scene ? `/api/pedagogies?scene=${scene}` : '/api/pedagogies';
  const response = await apiClient.get<ApiResponse<{ pedagogies: PedagogyModel[]; total: number }>>(url);
  return response.data;
};
 ...
 * 获取所有可用布局
 */
export const listLayouts = async (): Promise<ApiResponse<{ layouts: Array<{ id: string; name: string; description: string; image_slots: number }>; total: number }>> => {
  const response = await apiClient.get<ApiResponse<{ layouts: any[]; total: number }>>('/api/layouts');
  return response.data;
};
...
): Promise<ApiResponse<HTMLRenderResult>> => {
  const response = await apiClient.post<ApiResponse<HTMLRenderResult>>(
    `/api/projects/${projectId}/render/html`,
    {
      template_id: templateId || 'theory_professional',
      pedagogy_method: pedagogyMethod || 'five_step'
    }
  );
  return response.data;
};
 ...
 * @param subject 学科领域
 */
export const generateSlotImages = async (
  projectId: string,
  imageSlots: ImageSlotRequest[],
  subject?: string
): Promise<ApiResponse<{ task_id: string; status: string; total_slots: number }>> => {
  const response = await apiClient.post<ApiResponse<{ task_id: string; status: string; total_slots: number }>>(
    `/api/projects/${projectId}/render/generate-images`,
    {
...
export const getImageGenerationStatus = async (
  ...
  const response = await apiClient.get<ApiResponse<any>>(
    `/api/projects/${projectId}/render/image-status/${taskId}`
  );
  return response.data;
};
 ...
 */
export const exportTemplatePPTX = async (
  projectId: string,
  templateId?: string,
  imagePaths?: Record<string, string>
): Promise<ApiResponse<{ pptx_path: string; download_url: string }>> => {
  const response = await apiClient.post<ApiResponse<{ pptx_path: string; download_url: string }>>(
    `/api/projects/${projectId}/export/pptx-template`,
    {
      template_id: templateId || 'theory_professional',
